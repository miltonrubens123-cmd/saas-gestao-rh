...
def criar_indices_performance():
    statements = [
        ...
        "CREATE INDEX IF NOT EXISTS idx_atendentes_usuario ON atendentes (usuario)",
        "CREATE INDEX IF NOT EXISTS idx_solicitacoes_empresa_status ON solicitacoes (empresa_id, status)",
        ...
        "CREATE INDEX IF NOT EXISTS idx_solicitacoes_data_criacao ON solicitacoes (data_criacao DESC)",
        "CREATE INDEX IF NOT EXISTS idx_anexos_solicitacao ON anexos (solicitacao_id)",
        "CREATE INDEX IF NOT EXISTS idx_convites_status ON convites_cadastro (status)",
        "CREATE INDEX IF NOT EXISTS idx_convites_email ON convites_cadastro (email)",
        "CREATE INDEX IF NOT EXISTS idx_filiais_empresa_ativo ON filiais (empresa_id, ativo)",
        "CREATE INDEX IF NOT EXISTS idx_setores_empresa_ativo ON setores (empresa_id, ativo)",
        ...
        "CREATE INDEX IF NOT EXISTS idx_colaboradores_empresa_status ON colaboradores (empresa_id, status)",
        "CREATE INDEX IF NOT EXISTS idx_colaboradores_filial ON colaboradores (filial_id)",
        ...
        "CREATE INDEX IF NOT EXISTS idx_documentos_sst_empresa_status ON documentos_sst (empresa_id, status)",
        "CREATE INDEX IF NOT EXISTS idx_documentos_sst_empresa_vencimento ON documentos_sst (empresa_id, data_vencimento)",
        "CREATE INDEX IF NOT EXISTS idx_documentos_sst_filial ON documentos_sst (filial_id)",
        "CREATE INDEX IF NOT EXISTS idx_documentos_sst_colaborador ON documentos_sst (colaborador_id)",
        "CREATE INDEX IF NOT EXISTS idx_eventos_revisao_sst_empresa_data ON eventos_revisao_sst (empresa_id, data_evento DESC)",
    ]
...
RUN_DB_INDEX_BOOTSTRAP = os.getenv("RUN_DB_INDEX_BOOTSTRAP", "true").lower() == "true"

if RUN_DB_BOOTSTRAP:
    criar_tabelas()

if RUN_DB_INDEX_BOOTSTRAP:
...
def limpar_caches_aplicacao():
    ...
    try:
        ...
        pass
    try:
        obter_atendentes_ativos.clear()
    except Exception:
        pass
    try:
        obter_todos_atendentes.clear()
    except Exception:
        pass
    try:
        obter_clientes_ativos.clear()
    except Exception:
        pass
    try:
        listar_empresas_ativas.clear()
    except Exception:
        pass
    try:
        listar_empresas_resumo.clear()
    except Exception:
        pass
    try:
        obter_colaboradores_dashboard.clear()
    except Exception:
        pass
    try:
        obter_colaboradores_quadro.clear()
    except Exception:
        pass
    try:
        obter_documentos_sst_painel.clear()
    except Exception:
        pass
...
@st.cache_data(ttl=30)
def obter_colaboradores_dashboard(empresa_id):
    return conn.execute(
        """
        SELECT
            c.id,
...
@st.cache_data(ttl=30)
def obter_colaboradores_quadro(empresa_id):
    return conn.execute(
        """
        SELECT
...
@st.cache_data(ttl=30)
def obter_documentos_sst_painel(empresa_id):
    return conn.execute(
        """
        SELECT
            d.id,
...
def render_documento_sst_arquivo(doc_id, prefixo="sst_arquivo"):
    arquivo_doc = conn.execute(
        """
        SELECT arquivo, arquivo_nome
        FROM documentos_sst
        WHERE id = %s
    ...
    if not arquivo_doc or not arquivo_doc.get("arquivo"):
        return

    nome_arquivo = arquivo_doc.get("arquivo_nome") or f"documento_{doc_id}"
    ext = Path(nome_arquivo).suffix.lower()
    mime_map = {
        ".pdf": "application/pdf",
        ".png": "image/png",
    ...
    st.download_button(
        label="Baixar arquivo",
        data=arquivo_doc["arquivo"],
        file_name=nome_arquivo,
        mime=mime_map.get(ext, "application/octet-stream"),
        key=f"{prefixo}_{doc_id}",
        use_container_width=True,
    )
...
if menu == "Nova Solicitação":
    ...
    dados = obter_colaboradores_dashboard(empresa_id)
    ...
    if df.empty:
        ...
        docs_sst = conn.execute(
            ...
            SELECT
                ...
                ) AS revisao_necessaria,
                COUNT(*) FILTER (
                    WHERE revisao_necessaria = FALSE
    ...
    empresa_id = get_empresa_contexto()
    ###############
    dados = obter_colaboradores_quadro(empresa_id)
    df = pd.DataFrame(dados) if dados else pd.DataFrame()
    if df.empty:
    ...
    resumo = conn.execute(
        ...
        SELECT
            ...
            COUNT(*) FILTER (WHERE revisao_necessaria = TRUE) AS revisao,
            COUNT(*) FILTER (
                WHERE revisao_necessaria = FALSE
